...
public class NTriples implements Serializer {
	...
	private String convertProductType(ProductType pType)
	{
		...
		//dc:date XSD.Integer instead of XSD.Date
		int fake = (int)(Math.random() * 4) + 2;
		String fakeStr = Integer.toString(fake);
		result.append(createTriple(
				subjectURIREF,
				createURIref(DC.date),
				createDataTypeLiteral(fakeStr, createURIref(XSD.Date))));
